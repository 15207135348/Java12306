...
public class JSParser {
    ...
    public static JSFunction getFunc(String script, String funcName) {
        String startWith1 = String.format("function %s(", funcName);
        String func = getFuc_(script, startWith1);
        if (func != null) {
            return new JSFunction(funcName, func);
        }
    ...
    public static List<JSFunction> getFuncs(String script) {
        String temp = script;
        List<JSFunction> funcs = new ArrayList<>();
        int index;
        String name;
        JSFunction jsFunc;
        while (true) {
